<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(0, 204, 0, 1)","textAlign":"left"}'>餐品分类：</view>
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.canpinfenlei}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusscanpinfenlei/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('canpinfenlei', _this.detail);
